...
| AI 预分析 | `/analyze` | 上传图片/视频URL → GLM-5V-Turbo 多模态分析场景、物体、情绪曲线、推荐广告窗口 |
...
#   OPENAI_MODEL=glm-4.7-flashx          # 文本模型（推荐）
#   OPENAI_VISION_MODEL=glm-5v-turbo     # 视觉模型（推荐·支持视频理解）
...
| 模型角色 | 环境变量 | 用途 | 推荐（智谱） |
|----------|----------|------|-------------|
| 文本模型 | `OPENAI_MODEL` | 过渡语/回归语/情绪聚合/修复引擎 | `glm-4.7-flashx`（推荐） |
| 视觉模型 | `OPENAI_VISION_MODEL` | `/analyze` 多模态场景+视频分析 | `glm-5v-turbo`（推荐·支持视频） |
...
│       ├── repair/              # AI 广告修复
│       ├── analyze/             # 场景分析（图片+视频URL / 视觉模型 / Mock 三模式）
│       └── status/              # LLM 模式状态
├── components/                  # UI 组件
├── data/
...
3. **LLM 重排**：`glm-4.7-flashx` 对 top 候选做精排，输出评分 + 理由
...
- [x] 首页产品介绍 + 3D IP 画廊
- [x] AI 预分析页（上传图片 / 视频 URL → GLM-5V-Turbo 多模态分析 / 预设快速演示）
...
- [x] 四模型架构（GLM-4.7-FlashX 文本 / GLM-5V-Turbo 视觉+视频 / embedding-3 向量 / GLM-4-Long 长上下文）
