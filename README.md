...
| AI 预分析 | `/analyze` | 上传图片/视频 → AI 分析场景、物体、情绪曲线、推荐广告窗口 |
| 品牌匹配 | `/match` | 可视化 AI 从广告库选品过程：扫描场景 → 逐一评估 → 选定最佳 |
| 对比演示 | `/compare` | 左右分屏：传统投放（高潮硬切不相关品牌）vs Echo（谷底+匹配品牌+过渡语+回归语） |
...
#   OPENAI_BASE_URL=https://open.bigmodel.cn/api/paas/v4  (智谱示例)
...
**LLM 兼容**：任何 OpenAI-Compatible API（智谱 GLM-4 / DeepSeek / Kimi / 通义千问 / OpenAI）。
...
│   └── prompts.ts               # 范闲体 Prompt 模板 + few-shot
└── lib/
...
- [x] 首页产品介绍 + 3D IP 画廊
- [x] AI 预分析页（上传/预设 → 场景识别/物体检测/情绪曲线/广告窗口推荐）
- [x] 品牌匹配页（广告库逐一评估动画 + 契合度排名）
- [x] 对比演示页 — 左屏：传统投放（高潮硬切护舒宝，品类不匹配）
- [x] 对比演示页 — 右屏：Echo 投放（过渡语 → 飞鹤广告 → 回归语）
- [x] AI 决策面板（5步决策流程可视化）
- [x] 流式文案生成（过渡语 + 回归语通过 /api/tone 流式输出）
- [x] Mock 模式全功能兜底
- [x] 暗色主题 + 液态玻璃设计体系
...
- [ ] **接入真实多模态 API**：预分析页目前用 Mock 数据，需接入 GLM-4V 或 GPT-4o 做真实视觉分析
- [ ] **过渡语/回归语质量调优**：当前用通用 tone API，可针对"过渡"和"回归"两个场景定制专用 prompt
- [ ] **对比页动画打磨**：过渡语卡片出现/消失的过渡动画可以更流畅
- [ ] **品牌匹配页交互增强**：支持用户手动选择品牌查看不同匹配结果
- [ ] **情绪曲线可视化增强**：预分析页的 SVG 图表可以加动画、hover tooltip
- [ ] **移动端适配**：当前以桌面端优先，移动端部分布局需要调整
...
3. **广告效果仪表盘**：投放后的数据反馈面板（跳过率、完播率、转化率对比）
...
3. Environment Variables 添加 `OPENAI_API_KEY` / `OPENAI_BASE_URL` / `OPENAI_MODEL`
4. Deploy
